...
function delegationFunc(e) {
  ...
  while (!elem.getAttribute("data-name")) {
    ...
    // 데이터네임을 찾으며 올라가다 BODY까지 값이 없아면 null 반환
  ...
  if (elem.matches('[data-name="heartbeat"]')) {
    ...
    var content = document.querySelector("#add-comment-post37>input[type=text]")
      .value;
    ...
    $.ajax({
      ...
      success: function (data, textStatus, jqXHR) {
        document
          ...
          .insertAdjacentHTML("afterbegin", data); // data를 element 안에 가장 첫번째 child 위치에 생성
      },
      error: function (request, status, error) {
        alert(
          "code:" +
...
function resizeFunc() {
  ...
  // 800px이하로내려가게되면 실행됨 800이하로내려가면 순간적으로 그 이하 width가 style태그로 잡힘
...
function scrollFunc() {
  // pageYOffset은 스크롤이 일어나지않은 첫 페이지 높이는 계산에 들어가지않기때문에 그부분은 더해주어야함
